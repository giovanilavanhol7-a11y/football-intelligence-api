...
def info():
    return jsonify({
        ...
        "active_2026_27_stats": [
            "goals", "corners", "yellow_cards", "red_cards"
        ],
        "analysis_windows": [5, 10],
        "reserved_future_stats": [
            "xg", "shots", "shots_on_target", "fouls_committed"
        ],
        "modules": [
            "partida", "historico", "ultimos_5", "ultimos_10",
            "casa_fora", "frequencias", "produzido_x_cedido",
            "pre_match", "pre_match_dual_5_10", "confidence_score",
            "ranking_oportunidades", "temporada_2026_27",
            "fixtures_2026_27", "fixture_raw_2026_27",
            "fixture_stats_2026_27", "team_history_2026_27",
            "prematch_2026_27", "prematch_dual_2026_27"
        ],
        "future_modules": [
            "jogadores", "arbitro", "contexto", "storage_2026_27"
        ],
        "integrity": {
            "missing_data": None,
...
def status_2627():
    try:
        ...
        return jsonify({"api": API_NAME, "version": VERSION, **result})
    except Exception as error:
        return jsonify({
            "status": "error", "season": "2026/27", "error": str(error)
        }), 500
...
def fixtures_2627():
    try:
        ...
        result = fixtures_2627_by_date(date)
        return jsonify({"status": "ok", **result})
    except ValueError as error:
        return jsonify({"status": "invalid_request", "error": str(error)}), 400
    except requests.exceptions.RequestException as error:
        return jsonify({
            "status": "source_error", "source": "Sportmonks", "error": str(error)
        }), 502
    except Exception as error:
...
def fixture_2627_raw(fixture_id):
    try:
        ...
        if not fixture:
            return jsonify({
                "status": "not_found", "fixture_id": fixture_id
            }), 404
        return jsonify({
            "status": "ok",
            "season": "2026/27",
        ...
        return jsonify({
            "status": "source_error", "source": "Sportmonks",
            "fixture_id": fixture_id, "error": str(error)
        }), 502
    except Exception as error:
        return jsonify({
            "status": "error", "fixture_id": fixture_id, "error": str(error)
        }), 500
...
def fixture_2627_stats(fixture_id):
    try:
        result = analyze_fixture_2627(fixture_id)
        if not result:
            return jsonify({
                "status": "not_found", "fixture_id": fixture_id
            }), 404
        ...
        return jsonify({
            ...
            "coverage": match_coverage(result),
            "integrity": {
...
def team_history_2627(team_id):
    try:
        ...
        venue = request.args.get("venue", default="all", type=str)
        before_date = request.args.get("before_date", default=None, type=str)

        if season_id is None:
            return jsonify({
                "status": "invalid_request", "error": "season_id é obrigatório"
            }), 400
        if limit not in [5, 10]:
            return jsonify({
                "status": "invalid_request", "error": "limit deve ser 5 ou 10"
            }), 400
        ...
        return jsonify({
            "status": "ok", "api": API_NAME, "version": VERSION, **result
        })
    except ValueError as error:
        return jsonify({"status": "invalid_request", "error": str(error)}), 400
    except requests.exceptions.RequestException as error:
        return jsonify({
            "status": "source_error", "source": "Sportmonks",
            "team_id": team_id, "error": str(error)
        }), 502
    except Exception as error:
        return jsonify({
            "status": "error", "team_id": team_id, "error": str(error)
        }), 500
...
def prematch_2627():
    try:
        ...
        if home_team_id is None:
            return jsonify({
                "status": "invalid_request", "error": "home_team_id é obrigatório"
            }), 400
        if away_team_id is None:
            return jsonify({
                "status": "invalid_request", "error": "away_team_id é obrigatório"
            }), 400
        if season_id is None:
            return jsonify({
                "status": "invalid_request", "error": "season_id é obrigatório"
...
# =========================================================
# NOVO: PRÉ-JOGO DUAL 5 + 10
# =========================================================

@app.route("/api/v1/2627/prematch/dual")
def prematch_dual_2627():
    try:
        home_team_id = request.args.get("home_team_id", type=int)
        away_team_id = request.args.get("away_team_id", type=int)
        season_id = request.args.get("season_id", type=int)
        ...
        result = analyze_prematch_dual_2627(
            home_team_id=home_team_id,
            away_team_id=away_team_id,
            season_id=season_id,
            before_date=before_date
...
def competitions():
    try:
        ...
        return jsonify({
            "status": "ok", "total": len(result), "competitions": result
        })
    except requests.exceptions.RequestException as error:
        return jsonify({"status": "source_error", "error": str(error)}), 502
    except Exception as error:
        return jsonify({"status": "error", "error": str(error)}), 500
...
def matches(competition_id, season_id):
    try:
        data = get_matches(competition_id, season_id)
        result = []
        for match in data:
            ...
            result.append({
                ...
                "home": home.get("home_team_name"),
                "away": away.get("away_team_name"),
...
def match_stats(match_id):
    try:
        ...
        return jsonify({
            "status": "source_error", "match_id": match_id, "error": str(error)
        }), 502
    except requests.exceptions.RequestException as error:
        return jsonify({
            "status": "source_error", "match_id": match_id, "error": str(error)
        ...
        return jsonify({
            "status": "error", "match_id": match_id, "error": str(error)
        }), 500
...
def team_history():
    try:
        ...
        if competition_id is None:
            return jsonify({
                "status": "invalid_request", "error": "competition_id é obrigatório"
            }), 400
        if season_id is None:
            return jsonify({
                "status": "invalid_request", "error": "season_id é obrigatório"
        ...
        if not team:
            return jsonify({
                "status": "invalid_request", "error": "team é obrigatório"
            }), 400
        if limit not in [5, 10]:
            return jsonify({
                "status": "invalid_request", "error": "limit deve ser 5 ou 10"
        ...
        if venue not in ["all", "home", "away"]:
            return jsonify({
                "status": "invalid_request", "error": "venue deve ser all, home ou away"
            }), 400
...
def prematch():
    try:
        ...
        if not home_team:
            return jsonify({
                "status": "invalid_request", "error": "home é obrigatório"
            }), 400
        if not away_team:
            return jsonify({
                "status": "invalid_request", "error": "away é obrigatório"
            }), 400
        if limit not in [5, 10]:
            return jsonify({
                "status": "invalid_request", "error": "limit deve ser 5 ou 10"
